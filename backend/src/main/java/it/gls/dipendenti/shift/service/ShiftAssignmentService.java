...
public class ShiftAssignmentService {
     ...
     */
    @Transactional
    public int generateAssignmentsForDateRange(LocalDate startDate, LocalDate endDate) {
        if (startDate == null || endDate == null) {
            throw new IllegalArgumentException("Start and end dates cannot be null");
        }
        if (endDate.isBefore(startDate)) {
        ...
        // Delete existing auto-generated assignments in the range
        assignmentRepository.deleteAutoGeneratedByDateRange(startDate, endDate);

        int createdCount = 0;
        LocalDate currentDate = startDate;

        while (!currentDate.isAfter(endDate)) {
            createdCount += generateAssignmentsForDate(currentDate);
     ...
     */
    @Transactional
    public int generateAssignmentsForDate(LocalDate date) {
        if (date == null) {
            throw new IllegalArgumentException("Date cannot be null");
        }

        List<ShiftAssociation> associations = associationRepository.findByDayOfWeek(date.getDayOfWeek());
        int createdCount = 0;

        for (ShiftAssociation association : associations) {
            // Check if assignment already exists (manual or auto-generated)
            if (assignmentRepository.existsByEmployeeIdAndDate(association.employeeId(), date)) {
                continue;
            }
            ...
            ShiftAssignment adjustedAssignment = adjustForAbsences(assignment);

            if (adjustedAssignment != null) {
                assignmentRepository.save(adjustedAssignment);
                createdCount++;
            }
        }
    ...
    private void checkForOverlappingShifts(ShiftAssignment assignment) {
        List<ShiftAssignment> existingAssignments = assignmentRepository
                .findByEmployeeId(assignment.employeeId())
                .stream()
                .filter(a -> a.date().equals(assignment.date()))
